...
function hideGameBlocks() {
    ...
    game_install_button.removeClass('hidden')
...
function drawMainButtonCountdown(sec, disableMainButton) {
	...
	if(sec>0) {
		...
		$('#game_main').localize()
		new Countdown({
			uniqueKey: 'playButtonCountDown_'+window.games.selectedGameName,
			...
			onCounterEnd: function(uniqueKey){
                if(uniqueKey === 'playButtonCountDown_'+window.games.selectedGameName) {
                ...
                if(window.gamePurhcasesIsSyncing[window.games.selectedGameName]) {
                    $('[data-type="region"]').parent().addClass('hidden')
                }
			} // final action
		}).start()
	} else {
...
function drawSyncPurchaseWithCountdown(sec) {
    window.gamePurhcasesIsSyncing[window.games.selectedGameName] = true
    setGameEdition(i18next.t("Synchronizing purchases..."))
    game_main.find('[data-type="region"]').parent().addClass('hidden')
    drawMainScreenGameState(window.games.getSelectedGame())
...
function changePage() {
    ...
    if(window.settings.externalProvider === 'steam' && page === 'news') {
        page = 'main'
    }
